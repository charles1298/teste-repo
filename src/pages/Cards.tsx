...
const CardItem = ({ card, isOwned, onToggleOwn }: { card: SupportCard, isOwned: boolean, onToggleOwn: () => void }) => {
  const getRarityColor = (rarity: string) => {
    switch(rarity) {
      ...
      default: return 'text-gray-400 bg-zinc-800 border-gray-500';
    }
  };
  ...
  return (
    <motion.div
      ...
      className={clsx(
        "card group", 
        isOwned ? "border-uma-pink shadow-md" : "border-pink-900/30"
      )}
    >
      ...
      <div className="flex flex-col flex-grow text-center">
        <h3 className="text-sm font-bold text-gray-100 line-clamp-2 h-10 mb-2">{card.name}</h3>
        <button
          onClick={onToggleOwn}
          className={clsx(
            "mt-auto w-full py-2 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-colors",
            isOwned 
              ? "bg-pink-900/40 text-uma-pink hover:bg-pink-200" 
    ...
    <div className="space-y-6 pb-10">
      <div className="bg-zinc-900 rounded-3xl p-6 shadow-sm border border-pink-900/30 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-slate-300 font-medium">Veja e adicione as cartas SSR ao seu deck!</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 lg:gap-6">
        ...
        {filteredCards.length === 0 && (
          <div className="col-span-full py-12 text-center text-slate-300 bg-zinc-900 rounded-3xl border-2 border-dashed border-pink-900/50">
