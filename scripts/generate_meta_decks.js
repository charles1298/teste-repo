...
const cardsSrc = fs.readFileSync(path.join(process.cwd(), 'src/data/cards.ts'), 'utf-8');
function getActualType(cId) {
  const match = cardsSrc.match(new RegExp(`"id":\\s*"${cId}"[\\s\\S]*?"type":\\s*"([^"]+)"`));
  const rawType = match ? match[1] : 'Speed';
  if (rawType === 'Friend/Group' || rawType === 'Group' || rawType === 'Unknown') return 'Friend';
  return rawType;
}
...
RAW.forEach((char) => {
  ...
  const poolCards = Array.from(allCardsForChar).map(cId => {
    const actualType = getActualType(cId);
    return {
      id: cId,
      reason: localizedTypes[actualType] || actualType,
      type: actualType
    };
  });
